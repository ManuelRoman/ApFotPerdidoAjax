...
/**
 * Clase que encapsula el proceso de finalizar el juego a petición del usuario, responde a una petición ajax.
 */
public class AccionTerminar implements Accion{
	...
	public boolean ejecutar(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			posicion = daoConsultas.getPosicion(usuario);
		...
		if(error==null){
			if(datosJuego != null){
				respuesta.add((int) datosJuego.getPorAciertosSesion());
